...
public class drive extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(isStarted() && !isStopRequested()){
            ...
            if (gamepad2.dpad_up){outtakeExtendTarget = 3000;}
            else if (gamepad2.dpad_down){outtakeExtendTarget = 715;}
            else if (gamepad2.dpad_left){outtakeExtendTarget = 0;}

            if (gamepad2.right_trigger > 0){outtake.clawClose();}
            else if (gamepad2.left_trigger > 0){outtake.clawOpen();}
